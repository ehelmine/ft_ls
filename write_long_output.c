...
/*   Updated: 2021/06/02 17:20:22 by ehelmine         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	struct group *grp)
{
	if (grp == NULL && pwd != NULL)
		ft_printf("  %*i %s  %llu  %*lli ", all->links_len, buf.st_nlink,
			pwd->pw_name, (unsigned long long)buf.st_gid, all->size_len
			+ (all->group_len - ft_check_int_len(buf.st_gid)), buf.st_size);
	else if (grp != NULL && pwd == NULL)
		ft_printf("  %*i %llu  %s  %*lli ", all->links_len, buf.st_nlink,
			(unsigned long long)buf.st_uid, grp->gr_name, all->size_len
			+ (all->group_len - ft_strlen(grp->gr_name)), buf.st_size);
	else if (grp == NULL && pwd == NULL)
		ft_printf("  %*i %llu  %llu  %*lli ", all->links_len, buf.st_nlink,
			(unsigned long long)buf.st_uid, (unsigned long long)buf.st_gid,
			all->size_len + (all->group_len - ft_check_int_len(buf.st_gid)),
			buf.st_size);
	else
		ft_printf("  %*i %s  %s  %*lli ", all->links_len, buf.st_nlink,
			pwd->pw_name, grp->gr_name, all->size_len + (all->group_len
				- ft_strlen(grp->gr_name)), buf.st_size);
}
